...
### 3. 动态品牌配置 (Dynamic Branding)
- **实时自定义**：现在支持在“系统设置”中实时修改 **系统名称** 和 **应用图标**。
- **即时生效**：上传图片作为 Logo 后，软件窗口左上角图标、侧边栏标题、登录页 logo 以及浏览器标签页标题都会立即同步更新。
- **本地持久化**：自定义配置安全存储在本地数据库中，每次启动都会自动加载。

### 4. 数据隔离与存储
- **本地存储路径**：数据不再存储在程序目录下，而是遵循 Windows 标准。每个人、每个公司的数据库都是完全独立的。
- **具体路径**：`%APPDATA%\com.tonwin.financial\financial.db`
- **优化项目结构**：移除了 Express、Electron 及其相关的服务端库，代码库更精简。
