...
const InvoiceModal = ({ isOpen, onClose, transaction, customer }) => {
    ...
    const handleDownload = async (type = 'pdf') => {
        ...
        try {
            ...
            // Capture the dedicated hidden element which is at 1:1 scale
            const canvas = await html2canvas(element, {
                scale: 2, // 2x for high quality without being excessively heavy
                useCORS: true,
                logging: false,
                backgroundColor: '#ffffff',
                // This ensures we capture at the "desktop" layout width regardless of mobile viewport
                windowWidth: 1200
            });
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="relative w-full max-w-2xl bg-slate-100 dark:bg-slate-900 rounded-xl shadow-2xl mx-4 animate-in zoom-in-95 duration-200 border border-slate-200 dark:border-slate-800 flex flex-col max-h-[90vh]">
                ...
                {/* Header Actions */}
                <div className="flex items-center justify-between p-4 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 rounded-t-xl relative z-10">
                    <h2 className="text-lg font-bold text-slate-900 dark:text-white">电子收据预览</h2>
                    <div className="flex items-center gap-2">
                        <button
                            onClick={() => handleDownload('png')}
                            disabled={isGenerating}
                            className="flex items-center gap-2 px-3 py-1.5 bg-slate-100 dark:bg-800 text-slate-700 dark:text-slate-300 text-sm font-medium rounded-lg hover:bg-slate-200 transition-colors disabled:opacity-50"
                        >
                            保存图片
                        </button>
                        <button
                ...
                {/* Hidden Capture Area (1:1 Scale) */}
                <div style={{ position: 'absolute', top: '-9999px', left: '-9999px', width: '1200px' }}>
                    <div ref={invoiceRef}>
                        <InvoicePaper transaction={transaction} customer={customer} settings={settings} />
                    </div>
                </div>

                {/* Visible Preview Area (Scaled) */}
                <div className="flex-1 overflow-y-auto overflow-x-hidden p-0 md:p-6 bg-slate-100 dark:bg-slate-950 flex justify-center">
                    <div className="transform scale-[0.42] sm:scale-75 md:scale-100 origin-top transition-transform duration-200 mt-4 md:mt-0 h-[500px] sm:h-[900px] md:h-auto">
                        <InvoicePaper transaction={transaction} customer={customer} settings={settings} isPreview />
                    </div>
                </div>
            </div>
        </div>
...
const InvoicePaper = ({ transaction, customer, settings, isPreview = false }) => (
    <div
        className={`bg-white text-slate-900 w-[210mm] min-h-[297mm] p-[20mm] ${isPreview ? 'shadow-lg' : ''} mx-auto`}
